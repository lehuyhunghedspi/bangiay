...
do{
    if($_FILES['anhminhhoa_'.$i]['name'] != NULL){ // Đã chọn file
        ...
        if($_FILES['anhminhhoa_'.$i]['type'] == "image/jpeg"
        || $_FILES['anhminhhoa_'.$i]['type'] == "image/png"
        || $_FILES['anhminhhoa_'.$i]['type'] == "image/gif"){
        // là file ảnh
        // Tiến hành code upload    
            if($_FILES['anhminhhoa_'.$i]['size'] > 10485760){
...
<script>
		function xetdefaultcacmau(){
			...
			for(i=0;i<colors.length;i++){
				colors[i].children[0].children[1].src="../cacmau/"+colors[i].children[1].value+".png";
			}
		}
		xetdefaultcacmau();
		</script>
...
function doimau(select){
	...
	img.src="../cacmau/"+select.value+".png";

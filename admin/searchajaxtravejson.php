 ...
 function mang_sizesoluong($conn,$idgiay){
	...
	while($mau_row=mysqli_fetch_array($mau)){
		...
		while($size_row=mysqli_fetch_array($size)){
			...
			$qr2='SELECT sum(`giaodich`.soluong) as soluongdaban FROM `mau`,`sizemau`,`giaodich` WHERE giay_idgiay='.$idgiay.' and mau_idmau=idmau and sizemau_idsizemau=idsizemau and idmau='.$mau_row['idmau'].' and `sizemau`.size='.$size_row["size"].";";
			//echo $qr2;
			$soluongdaban=mysqli_query($conn,$qr2);
			if(isset($soluongdaban))
			{
				$soluongdaban_row=mysqli_fetch_array($soluongdaban);
				$phantu_size["soluong"]=$size_row["soluongbandau"]-$soluongdaban_row["soluongdaban"];
			}
			else{$phantu_size["soluong"]=$size_row["soluongbandau"];}
